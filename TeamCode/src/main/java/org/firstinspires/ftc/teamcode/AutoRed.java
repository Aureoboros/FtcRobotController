...
@Autonomous(name="AutoFRRed", group="Autonomous")
public class AutoRed extends LinearOpMode {
    ...
    private static final int TARGET_APRILTAG_ID = 20;  // Red alliance goal
    ...
    // Red alliance spike marks (right side of field, mirrored from red)
    private static final double RED_MIDDLE_SPIKE_X = -3.0;  // Middle spike mark (mirrored)
    private static final double RED_MIDDLE_SPIKE_Y = -1.0;  // Below center line
    ...
    // Park position (red observation zone - mirrored)
    ...
    private static final double RED_DEFAULT_START_X = -4.0;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        telemetry.addLine("========================================");
        telemetry.addLine("RED ALLIANCE: 2 SPIKE MARKS AUTO");
        telemetry.addLine("========================================");
        telemetry.addLine();
        ...
        while (!isStarted() && !isStopRequested() && !positionDetected && detectionAttempts < maxAttempts) {
            ...
            telemetry.addData("  1. Middle Spike", "X: %.1f, Y: %.1f", RED_MIDDLE_SPIKE_X, RED_MIDDLE_SPIKE_Y);
            telemetry.addData("  2. Shoot Position", "X: %.1f, Y: %.1f", RED_SHOOT_X, RED_SHOOT_Y);
            telemetry.addData("  3. Top Spike", "X: %.1f, Y: %.1f", RED_TOP_SPIKE_X, RED_TOP_SPIKE_Y);
            telemetry.addData("  4. Shoot Position", "X: %.1f, Y: %.1f", RED_SHOOT_X, RED_SHOOT_Y);
            telemetry.addData("  5. Park", "X: %.1f, Y: %.1f", RED_PARK_X, RED_PARK_Y);
            telemetry.addLine("========================================");
            telemetry.update();
        ...
        telemetry.update();
        driveToPosition(RED_MIDDLE_SPIKE_X, RED_MIDDLE_SPIKE_Y, 0);

        // 2. Intake balls from middle spike
        telemetry.addData("Step 2", "Intaking Middle Spike");
        telemetry.update();
        intakeBalls(3.5, RED_MIDDLE_SPIKE_X, RED_MIDDLE_SPIKE_Y);  // 3.5 seconds to intake
        ...
        telemetry.update();
        driveToPosition(RED_TOP_SPIKE_X, RED_TOP_SPIKE_Y, 0);

        // 7. Intake balls from top spike
        telemetry.addData("Step 7", "Intaking Top Spike");
        telemetry.update();
        intakeBalls(3.5, RED_TOP_SPIKE_X, RED_TOP_SPIKE_Y);
