...
@Autonomous(name = "AutoBlueBackStart", group = "FTC2025")
public class AutoBlueBackStart extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (opModeIsActive()) {
            ...
            // === ROTATE LEFT 45 DEGREES (using LT rotation pattern) ===
            telemetry.addLine("Rotating left 45 degrees...");
            telemetry.update();

            // Use LT rotation pattern from SVTestTeleop12nov.java (lines 960-967)
            // Pattern from SVTestTeleop12nov.java lines 960-967:
            // frontLeftMotor: ltPower (backward, positive)
            // backLeftMotor: -ltPower (backward, negative)
            // frontRightMotor: ltPower (forward, positive)
            // backRightMotor: -ltPower (forward, negative)
            // For left rotation (counter-clockwise), we use positive ltPower
            double ltPower = 0.5;  // Rotation power (limited to max 0.5 like in teleop)
