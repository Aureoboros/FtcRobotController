...
@TeleOp
public class testteleop12nov extends LinearOpMode {
    ...
    private static final double OVERSHOOT_ANGLE_DEGREES = 7.0;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            // ========== TRIGGERS - ROTATE PAST APRILTAG BY 7 DEGREES (BOTH GAMEPADS) ==========
            if (currentGamepad1.left_trigger > 0.5 || currentGamepad2.left_trigger > 0.5 ||
                currentGamepad1.right_trigger > 0.5 || currentGamepad2.right_trigger > 0.5) {
            ...
            if (driverOverride && autoRotating) {
                autoRotating = false;
                hasSeenTag = false;
            }
            ...
            if (autoRotating) {
                // ========== ROTATE PAST APRILTAG BY 7 DEGREES ==========
                ...
                if (detection != null) {
                    ...
                    // Check if we've passed the tag by 7 degrees
                    if (selectedAlliance == Alliance.RED && yawError < -OVERSHOOT_ANGLE_DEGREES) {
                        // For red, rotating right means yaw becomes more negative
                        rx = 0;
                        autoStatus = "COMPLETE ✓";
                        autoRotating = false;
                        hasSeenTag = false;
                    } else if (selectedAlliance == Alliance.BLUE && yawError > OVERSHOOT_ANGLE_DEGREES) {
                        // For blue, rotating left means yaw becomes more positive
                        rx = 0;
                        autoStatus = "COMPLETE ✓";
                        autoRotating = false;
                        hasSeenTag = false;
                        ...
                        if (selectedAlliance == Alliance.RED) {
                            rx = AUTO_ALIGN_ROTATION_SPEED;  // Rotate right
                        } else {
                            rx = -AUTO_ALIGN_ROTATION_SPEED;  // Rotate left
                        }
                        autoStatus = String.format("ROTATING (%.1f° past)", Math.abs(yawError));
                    }
                } else {
                    // Tag not visible
                    if (hasSeenTag) {
                        // We saw it before, keep rotating in same direction
                        if (selectedAlliance == Alliance.RED) {
                            rx = AUTO_ALIGN_ROTATION_SPEED;
                        } else {
                            rx = -AUTO_ALIGN_ROTATION_SPEED;
                        }
                        autoStatus = "TAG LOST - CONTINUING...";
                    } else {
                        // Haven't seen tag yet, search slowly
                        if (selectedAlliance == Alliance.RED) {
                ...
                // Allow manual translation during auto-rotation
                if (GAMEPAD1_ACTIVE) {
...
//Shoot sequence start
            ...
            //    sleep(3000);
            //    rampMotor.setPower(RAMP_MOTOR_POWER * 0.7);
            //}
...
//            telemetry.addData("Heading", "%.0f°", Math.toDegrees(botHeading));
            ...
            if (autoRotating) {
                telemetry.addData(">>> AUTO-ROTATE", "ACTIVE <<<");
            }
            telemetry.addLine();
