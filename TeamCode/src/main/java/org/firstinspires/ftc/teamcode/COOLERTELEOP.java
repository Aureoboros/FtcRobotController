...
public class testteleop13nov extends LinearOpMode {
    ...
    private static final double GAMEPAD2_MAX_POWER = 1.0;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (autoAligning) {
                ...
                // ========== TRIGGER-BASED TURNING (NEW) ==========
                // Check gamepad1 triggers first (priority)
                if (gamepad1TurnActive) {
                    if (currentGamepad1.right_trigger > TRIGGER_TURN_THRESHOLD) {
                        // Right trigger = turn right
                        rx = currentGamepad1.right_trigger;
                    } else if (currentGamepad1.left_trigger > TRIGGER_TURN_THRESHOLD) {
                        // Left trigger = turn left
                        rx = -currentGamepad1.left_trigger;
                    }
                    activeDriver = "DRIVER 1";
                    maxDrivePower = GAMEPAD1_MAX_POWER;
                }
                // Check gamepad2 triggers if gamepad1 not turning
                         ...
                         currentGamepad2.right_trigger > TRIGGER_TURN_THRESHOLD)) {
                    if (currentGamepad2.right_trigger > TRIGGER_TURN_THRESHOLD) {
                        rx = currentGamepad2.right_trigger;
                    } else if (currentGamepad2.left_trigger > TRIGGER_TURN_THRESHOLD) {
            ...
            if (Math.abs(rx) > JOYSTICK_DEADZONE) {
                // Pure turning mode (from second code)
                frontLeftPower = Range.clip(rx * FRONT_WHEEL_TURN_MULTIPLIER, -THRESH_WM_POWER_FORTURN, THRESH_WM_POWER_FORTURN);
                backLeftPower = Range.clip(rx, -THRESH_WM_POWER_FORTURN, THRESH_WM_POWER_FORTURN);
                frontRightPower = Range.clip(-rx * FRONT_WHEEL_TURN_MULTIPLIER, -THRESH_WM_POWER_FORTURN, THRESH_WM_POWER_FORTURN);
                backRightPower = Range.clip(-rx, -THRESH_WM_POWER_FORTURN, THRESH_WM_POWER_FORTURN);
            } else {
